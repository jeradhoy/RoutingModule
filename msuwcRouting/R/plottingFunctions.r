...
#
makeHydrographs <- function(flowData, flowData2, gaugeData, var=1, precip=NULL, spack=NULL, saveGraphs=F, plotTogether=F, interact=T, plotStats=T, plotSnowpack=F, plotPrecip=F, plotSeason=F, plotAnnual=F, dataMin=50, yLabel="Flow (m3/s)", timeStep=setupList$timeStep, gofs=NULL, titleStart=NULL){
    ...
    if(length(gauges) >= 1){
		...
		for(i in 1:length(gauges)){
			...
			if(plotSeason){
				...
				flow2 <- data.frame(flowData2[[var]][, names(gauges)[i]])
				flow2 <- aggregate(flow2, list(substring(rownames(flow2), 4)), FUN=sum)
				...
				lines(dates, flowData[[var]][, names(gauges)[i]], col="brown", lwd=2)
			...
			if(!plotSeason){
				...
				title(paste(titleStart, "Edge",names(gauges[i])), cex.main=1.3)
				legend("topleft", col=c("red", "black"), legend=c("Modelled LPJ-Guess Data", "Observations"), lty=1)
			}

			if(plotStats){
				if(is.null(gofs)){
					gofToPlot <<- CalcGOFs(flowData, gauges[i])
					#gofs <<- gofs[c(1,4,6,7,9,10,11,17),]
				} else {
					gofToPlot <- gofs[[names(gauges)[i]]]
				}
				
				legend("topright", legend=paste0(rownames(gofToPlot), " = ", gofToPlot), ncol=2, cex=1)
			}
