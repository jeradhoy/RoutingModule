...
#' @name GetGaugeData
#' @export
GetGaugeData <- function(edges, gauges, snappedGauges=NA, simStartDate=setupList$simStartDate, simEndDate=setupList$simEndDate, aggregateByMonth=T, maxDist=.005, idField=setupList$edgeIdField, checkGauges=F, varCode="00060"){

	if(!is.na(snappedGauges)){
		#gaugesSnapped <<- tryCatch(snapPointsToLines(gauges, edges, maxDist=maxDist, idField=idField), error=function(e){return(c())})
	} else gaugesSnapped  <- snappedGauges
	...
	names(dat) <- as.character(gaugesInBounds@data[i,1])
#
#	if(length(gaugesSnapped) > 0 && any(gaugesSnapped@data[,1] == gaugesInBounds@data[i,1])){
#	    names(dat) <- as.character(gaugesSnapped@data[gaugesSnapped@data[,1] == gaugesInBounds@data[i,1], 1])
#	}else{
#	    names(dat) <- NA
#	}
