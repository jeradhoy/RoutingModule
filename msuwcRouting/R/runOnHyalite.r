...
runHyalite <- function(jobId=NULL, objs=ls(envir=parent.frame()), packages=.packages(), updateMSUWC=T, user="jerad.hoy", cores=1, oneLine=F, overwrite=T, queue="priority", timeLimit="72:00:00"){
	...
	writeLines(c("#!/bin/sh",
				 ...
				 paste0("#SBATCH -p ", queue),
				 "#SBATCH --no-requeue",
				 paste0("#SBATCH -t ",timeLimit),
				 "",
	...
	system(paste0("ssh hyalite 'sbatch /home/", user, "/", jobName, "/sbatch_submitR.sh' &"))
